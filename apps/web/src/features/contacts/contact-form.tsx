...
import { CONTACT_LABELS, CONTACT_RESOURCES, useSaveContact } from '@/features/contacts/api';
...
export function ContactForm({ type, contact: customer }: { type: ContactType; contact?: ContactDto }) {
  const router = useRouter();
  const save = useSaveContact(type);
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      ...
      toast.success(customer ? `${labels.one} updated` : `${labels.one} created`);
      router.push(`/${CONTACT_RESOURCES[type]}/${saved.id}`);
    } catch (error) {
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="pb-20">
      <div className="space-y-6 rounded-xl border bg-card p-6">
        <FormSection title={`${labels.one} details`}>
          <Field label={`${labels.one} type`}>
            <RadioGroup
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field
              ...
              htmlFor="displayName"
              required
              hint={`How the ${labels.lower} appears on documents. Pick a suggestion or type your own.`}
        ...
        <FormSection title="Financial details">
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <Field
              ...
              hint={type === 'customer' ? 'What the customer already owed you when you started using the system.' : 'What you already owed this vendor when you started using the system.'}
        ...
        <FormSection title="Contact persons" description={`People you deal with at this ${labels.lower}.`}>
